...
try {
  const extrudeCmd = new ExtrudeFaces(mesh, selection, undefined, 0.5);
  extrudeCmd.execute();
...
function createSimpleBuilding() {
  ...
  for (let floor = 0; floor < 2; floor++) {
    ...
    const extrudeCmd = new ExtrudeFaces(buildingMesh, sel, undefined, 1.5);
    extrudeCmd.execute();
...
console.log('  3. selection.selectAllFaces() - Select geometry');
