...
/**
 * Command to apply Laplacian smoothing to selected vertices or entire mesh.
 * Laplacian smoothing moves each vertex towards the average position of its neighbors.
 */
export class LaplacianSmooth implements ICommand {
  private mesh: Mesh;
  private selectionManager: SelectionManager;
  private iterations: number;
  private lambda: number; // Smoothing factor (0.0 to 1.0)
  private smoothSelectedOnly: boolean;
  private preserveBoundary: boolean;
  
  // Store data for undo
  private smoothingData: SmoothingData[] = [];
  
  public readonly description: string;

  /**
   * Creates an instance of LaplacianSmooth command.
   * @param mesh - The mesh to smooth.
   * @param selectionManager - The selection manager.
   * @param iterations - Number of smoothing iterations (default: 1).
   * @param lambda - Smoothing factor 0.0-1.0 (default: 0.5).
  ...
  constructor(
    ...
    this.description = `Laplacian smooth ${targetDesc} (${iterations} iteration${iterations === 1 ? '' : 's'}, λ=${lambda.toFixed(2)})`;
  }
  ...
  execute(): void {
    ...
    if (verticesToSmooth.length === 0) {
      console.warn('LaplacianSmooth: No vertices to smooth.');
      return;
    }
    ...
    for (let i = 0; i < this.iterations; i++) {
      this.performSmoothingIteration(verticesToSmooth);
    }

    console.log(`LaplacianSmooth: Smoothed ${verticesToSmooth.length} vertices over ${this.iterations} iteration${this.iterations === 1 ? '' : 's'}.`);
  }
  ...
  private getVerticesToSmooth(): number[] {
    if (this.smoothSelectedOnly) {
      ...
      if (selectedVertices.size === 0) {
        console.warn('LaplacianSmooth: No vertices selected for smoothing.');
        return [];
      }
  ...
  /**
   * Performs one iteration of Laplacian smoothing.
   * @param verticesToSmooth - Array of vertex IDs to smooth.
   */
  private performSmoothingIteration(verticesToSmooth: number[]): void {
  ...
  /**
   * Gets statistics about the smoothing operation.
